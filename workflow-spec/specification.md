...
>
> 供团队成员与 AI 编码代理共同参考 ｜ v1.0

> 这是 [`specification.docx`](./specification.docx) 的可读 Markdown 版本。两份内容等价，团队 / 客户分享可直接用 docx，仓库内查阅与改动用本文件。
...
**产出**：一份尖锐问题与已识别风险的清单，大致按严重程度分组。每一条都要具体到能据此行动（「迁移过程中正在处理的请求会怎样？」而不是「我们考虑过边界情况了吗？」）。
