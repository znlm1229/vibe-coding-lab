...
>
> 供团队成员与 AI 编码代理共同参考 ｜ **v1.4**

> 这是 [`specification.docx`](./specification.docx) 的可读 Markdown 版本，也是**正本**。docx 通过 `pandoc specification.md -o specification.docx --toc --toc-depth=2` 重生成，与本文件同步（v1.4 升级后请重跑此命令）。
>
> **v1.4 关键变更**（在 v1.3 基础上累加）：(a) 规则五强化——每个 stage "done" 的定义显式扩展为「0X-stage.md 已填 + 已 commit + 人工关卡确认槽已被用户翻转」，不能只靠 chat 声称；(b) 新增规则六 "Resume from `workflow/` state"——AI 在已有 `workflow/` 的项目上启动时必须先从 md 文件里探测断点，不能从记忆里凭印象续做；(c) 9 个阶段详解节各加一句 Persistence，明示该写哪份 md / 用什么 commit prefix。详见 [§8 v1.4](#v14-2026-05-26)。
>
...
默认按顺序走完九个阶段，但要保持务实。六条规则约束着流程的推进方式（规则一-三是原始规则，规则四来自 v1.2，规则五来自 v1.3，规则六来自 v1.4）。
...
### 规则五（v1.3 新增，v1.4 强化）：每阶段 = 一份 md 文件，已填 + 已 commit + 关卡确认槽已翻转

工作流的九个阶段产出**不能只活在聊天上下文里**——session 结束后无法被新会话 / 新协作者 / 自己一周后看到。**所有 artifact 必须落到一个可读、可被未来的人和 AI 找到、可被 git 追踪的持久位置**。

**v1.4 严格读法**：一个阶段算"已完成"，当且仅当以下**三**条同时为真：

1. **文件已写**：对应的 `workflow/NNN-task/0X-stage.md` 已填了真实内容（不是模板占位），或在顶部写了 `> 已跳过 — 理由：<...>`（按规则一伸缩）
2. **已 commit**：该文件改动已进 git，按 §5 阶段 7 的 commit prefix 表（`stage-N:` / `task-TX:` / `fix(TX):`）打标
3. **人工关卡阶段（4 / 6 / 8 / 9）额外要求**：md 内"用户确认"槽位已由用户（不是 AI）翻转为 `⬜ 已确认`。AI 呈现 artifact、然后停下；翻转是人的工作

"我在 chat 里讲过了" / "我口头确认了" **不算**。md 文件没填 / 没 commit，阶段就没发生，无论 AI 在 chat 里说了多少。
...
### 规则六（v1.4 新增）：从 `workflow/` 状态恢复任务，不从记忆里凭印象续做

AI 的 session 记忆是易失的——经过上下文压缩、新 session、换机器、换协作者后，它对任务进展的"印象"不可靠。**唯一可靠的事实源是 git-tracked 的 `workflow/NNN-task/0X-stage.md` 文件**。

**因此 AI 在已有 `workflow/` 的项目上启动时（无论是用户说"继续上次"，还是开新对话），它的第一步动作必须是状态探测，而不是直接干活**：

```bash
# 1. 列出已存在的任务目录
ls workflow/                # → 001-foo/  002-bar/  _template/

# 2. 对每个 in-flight 任务读它的 9 个阶段文件
ls workflow/NNN-task-name/  # → 01-*.md ... 09-*.md
```

读完每份 md 后，按下表给每个阶段定状态：

| md 状态 | 阶段状态 |
|---|---|
| 文件缺失 / 只有模板占位 | **未开始** |
| 顶部 `> 已跳过 — 理由：...` | **已跳过**（按规则一伸缩，算完成） |
| 内容已填，"用户确认 → ⬜ 等待确认" | **已起草，关卡未过** |
| "用户确认 → ⬜ 已确认" 被勾上 | **已通过** |
| 阶段 1/3/5/7 无确认槽，内容非模板 | **已通过** |

**第一个不属于"已通过 / 已跳过"的阶段就是恢复点**（resume point）。AI 必须先把探测结果告诉用户，再问下一步：

> 例：「检测到 `workflow/002-account-rate-limit` —— Stage 1-4 已通过（SPEC 于 2026-05-20 确认），Stage 5 计划草稿已写但 Stage 6 尚未开始。要从 Stage 6 续做、回 Stage 5 修计划，还是切别的任务？」

**严格禁止两种行为**：

- **静默重做已通过阶段**——浪费精力，且若实现已进行就有 SPEC 静默漂移风险（§6 已有失败模式）
- **静默跳过未通过的人工关卡**——绕过流程内置的安全机制（违反规则二）

**为什么这条规则存在**：v1.3 落了「md 文件持久化」这一半，但还差「AI 启动时强制读它」的另一半。没有这一条，AI 容易在新 session 里凭印象往下做，对"上次走到哪了"的判断与文件实际状态脱节，等用户发现时已经在错的阶段走远了。
...
**触发**：v1.3 把「每个项目必须建 `workflow/`」立成规则，但用户使用过程中发现两个落地缺口：

1. **每个 stage "done" 的定义不够严**——v1.3 说"artifact 必须存在 md 里"，但没明示「md 必须填好 + 已 commit + 人工关卡确认槽必须被用户翻转」。结果 AI 容易出现"我在 chat 里讲完了 SPEC，所以 Stage 4 done 了 → 进 Stage 5"的滑坡，md 文件停留在模板状态。
2. **没有「从 `workflow/` 状态恢复任务」流程**——AI 在已有 `workflow/` 的项目上启动时，倾向于凭对话历史/记忆判断"上次走到哪了"，但 session 跨压缩 / 新会话 / 换人后这个判断不可靠。md 文件是事实源但 AI 没被强制要求读它。

**主要变更**：

1. **规则五强化（v1.4）**：一个 stage "done" = 三条同时满足（md 已填 + 已 commit + 关卡阶段的"用户确认"槽已被用户翻转）。AI chat 中口头声称不算。详见 [§3 规则五](#规则五v13-新增v14-强化每阶段--一份-md-文件已填--已-commit--关卡确认槽已翻转)。

2. **规则六新增（v1.4）**：AI 在已有 `workflow/` 的项目上启动时，**第一步必须是状态探测**（读 9 份 md 文件、按状态表判定每阶段状态、找出 resume point、告诉用户）。严禁静默重做已通过阶段或静默跳过未通过阶段。详见 [§3 规则六](#规则六v14-新增从-workflow-状态恢复任务不从记忆里凭印象续做)。

3. **`SKILL.md` 同步**：
   - frontmatter version 1.3 → 1.4，description 加恢复触发词（"继续 X / 接着上次 / resume / pick up"）
   - 顶部新增「How to start: new task or resume」整节，含状态判定表
   - 9 个 stage 详解每节末尾各加一句 "Persistence:"，明示该写哪份 md / 用什么 commit prefix
   - changelog 加 v1.4

4. **`templates/README.md` 同步**：加「恢复进行中的任务」段。

5. **`README.md` 同步**：增「下载与安装这个 skill」段（必须文件 vs 可选文件、三种安装方式、slash command wrapper 写法）。

6. **`specification.docx` 待同步**：本次 md 改完后需重跑 `pandoc specification.md -o specification.docx --toc --toc-depth=2`。
