...
4. 玩家可：
   a. 输入答案 + 点猜 → POST /api/check-answer → 显示对/错
      ...
      - 错 → **自动展示下一条线索**（v1.1 修订：消耗一条线索；之前 v1.0 是"不消耗"）；玩家可继续输入
...
| 版本 | 日期 | 触发 | 变更范围 |
|---|---|---|---|
| v1.0 | 2026-05-21 | Stage 4 初版（用户确认）| 完整 SPEC 落地 |
| v1.1 | 2026-05-22 | Stage 8 Human QA 期间用户体验反馈 | **Behavior Flow 1**：答错自动消耗一条线索（v1.0 是"错不消耗"，v1.1 改为"错自动消耗，玩家试探有代价"）。属于行为层变更但范围极小（仅 1 条 flow 规则），由用户在 Stage 8 期间提出。15 条 AC 不受影响（AC9 计分公式仍按"用了 k 条得 (6-k)*20 分"，"k"的语义不变）|
