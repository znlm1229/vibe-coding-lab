...
export async function submitStandaloneTurtleAnswer(input: {
  ...
  if (current.user_id !== input.userId || current.figure_id !== input.figure.id) {
  ...
  await input.db
    .prepare(
      "UPDATE turtle_sessions SET question_count = ?, answer_attempts_used = ?, completed = ?, won = ?, used_turtle = 1, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND completed = 0 AND answer_attempts_used < ?",
    )
    .bind(
      storedQuestionCount,
      attemptsUsed,
  ...
  return {
    ...
    completed,
    won,
    answer_attempts_used: attemptsUsed,
    answer_attempts_remaining: Math.max(0, STANDALONE_MAX_ANSWER_ATTEMPTS - attemptsUsed),
...
export async function persistFinishedGame(input: {
  ...
  return { persisted: true, score: effectiveScore, turtle_used: turtleUsed };
}
