...
- 后续候选任务:**004(海龟汤模式 + RAG 史料库 — 已启动)**、005(自定义域名 + 品牌化)、006(补 5 缺失皇帝 + V3 题库扩到 200 人 + 修 3 旧 figure warning)、007 候选(邮箱 magic link + 排行榜,原 004 占位 — 顺延)
...
| # | 任务 | 状态 | 备注 |
|---|---|---|---|
| 004 | [`004-turtle-soup-rag`](./workflow/004-turtle-soup-rag/) | 🟡 **进行中 — Stage 1（2026-05-26 起）** | 新玩法：海龟汤模式（用户提问 → AI 仅答「是/否/无关」→ 用户猜底）。技术核心：RAG（史料向量库 + 召回 + LLM 三态总结） |
